...
{
    public (bool IsValid, string? ErrorMessage) Validate(List<StrategyStage> stages,
        List<StrategyTransition> transitions)
    {
    ...
    public (bool IsValid, string? ErrorMessage) PreValidate(List<StrategyStage> stages,
        List<StrategyTransition> transitions)
    {
        var stageIds = GetStageIds(stages);
    ...
    private (bool IsValid, string? ErrorMessage) CheckEmptyness(List<StrategyStage> stages,
        List<StrategyTransition> transitions)
    {
        if (transitions.Count == 0 || stages.Count == 0) return (false, "List of transitions or stages is empty");
        return (true, null);
    }

    private (bool IsValid, string? ErrorMessage) CheckUnknownStagesInTransitions(List<StrategyStage> stages,
        List<StrategyTransition> transitions)
    {
